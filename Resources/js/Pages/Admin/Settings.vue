...
const formatMoney = (amount) => {
    ...
    if (currencyLeft.value) return `${currencyLeft.value}${value}`;
    if (currencyRight.value) return `${value} ${currencyRight.value}`;
    return value;
};
...
const submit = () => {
    ...
    saving.value = true;
    router.put(route('admin.shipping.flat.settings.update'), {
...
const resetAll = async () => {
    ...
    if (confirmed && storeTabsRef.value) {
        Object.assign(storeTabsRef.value.localSettings, JSON.parse(JSON.stringify(props.defaultSettings)));
    }
};
                        ...
                        <div class="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-lg p-5 text-white">
                            <div class="flex items-center space-x-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-2xl font-bold">{{ formatMoney(settings.cost) }}</p>
                                </div>
                            </div>
                            <div v-if="settings.free_shipping_threshold" class="pt-4 border-t border-white/20">
                                <div class="flex items-center space-x-2">
                                    <GiftIcon class="w-5 h-5 opacity-80" />
                                    <span class="text-sm opacity-80">
                                        {{ t('free_over', { amount: formatMoney(settings.free_shipping_threshold) }) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-blue-50 rounded-2xl p-5 border border-blue-100">
                            <div class="flex items-start space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-blue-700 mt-1">{{ t('help_text') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="p-6 space-y-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            @input="updateSetting('sort_order', parseInt($event.target.value) || 0)"
                                            min="0"
                                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                                            placeholder="0"
                                        />
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        @input="updateSetting('description', $event.target.value)"
                                        rows="3"
                            ...
                            <div class="p-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="bg-gray-50 rounded-xl p-5">
                                        ...
                                        <div class="relative">
                                            <span v-if="inputHasLeftCurrency" class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">{{ currencyLeft }}</span>
                                            <input
                                                type="number"
                                                :value="settings.cost"
                                                @input="updateSetting('cost', parseFloat($event.target.value) || 0)"
                                                step="0.01"
                                                min="0"
                                                :class="[
                                                    'w-full py-3 text-lg font-semibold border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors',
                                                    inputHasLeftCurrency ? 'pl-10' : 'pl-4',
                                                    inputHasRightCurrency ? 'pr-14' : 'pr-4',
                                                ]"
                                                placeholder="0.00"
                                            />
                                            <span v-if="inputHasRightCurrency" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 font-medium">{{ currencyRight }}</span>
                                        </div>
                                    ...
                                    <div class="bg-green-50 rounded-xl p-5 border border-green-100">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input
                                                ...
                                                @input="updateSetting('free_shipping_threshold', $event.target.value ? parseFloat($event.target.value) : null)"
                                                step="0.01"
                                                min="0"
                                                :class="[
                                                    'w-full py-3 text-lg font-semibold border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors bg-white',
                                                    inputHasLeftCurrency ? 'pl-10' : 'pl-4',
                                                    inputHasRightCurrency ? 'pr-14' : 'pr-4',
                                                ]"
